...
struct config_bool ConfigureNamesBool[] =
{
...

	{
		{"enable_presorted_aggregate", PGC_USERSET, QUERY_TUNING_METHOD,
			gettext_noop("Enables the planner's ability to produce plans that provide presorted input for ORDER BY / DISTINCT aggregate functions."),
			gettext_noop("Allows the query planner to build plans that provide presorted input for aggregate functions with an ORDER BY / DISTINCT clause.  When disabled, implicit sorts are always performed during execution."),
			GUC_EXPLAIN
		},
...

	{
		{"allow_alter_system", PGC_SIGHUP, COMPAT_OPTIONS_OTHER,
			...
			gettext_noop("Can be set to off for environments where global configuration changes should be made using a different method."),
...

	{
		{"fsync", PGC_SIGHUP, WAL_SETTINGS,
			...
			gettext_noop("The server will use the fsync() system call in several places to make sure that updates are physically written to disk. This ensures that a database cluster will recover to a consistent state after an operating system or hardware crash.")
		},
		&enableFsync,
		true,
		NULL, NULL, NULL
...
#ifdef DEBUG_NODE_TESTS_ENABLED
	{
		{"debug_copy_parse_plan_trees", PGC_SUSET, DEVELOPER_OPTIONS,
			gettext_noop("Set this to force all parse and plan trees to be passed through copyObject(), to facilitate catching errors and omissions in copyObject()."),
			NULL,
			GUC_NOT_IN_SAMPLE
		},
		&Debug_copy_parse_plan_trees,
		DEFAULT_DEBUG_COPY_PARSE_PLAN_TREES,
		NULL, NULL, NULL
	},
#endif
...
#ifdef DEBUG_NODE_TESTS_ENABLED
	{
		{"debug_write_read_parse_plan_trees", PGC_SUSET, DEVELOPER_OPTIONS,
			gettext_noop("Set this to force all parse and plan trees to be passed through outfuncs.c/readfuncs.c, to facilitate catching errors and omissions in those modules."),
			NULL,
			GUC_NOT_IN_SAMPLE
		},
...
#ifdef DEBUG_NODE_TESTS_ENABLED
	{
		{"debug_raw_expression_coverage_test", PGC_SUSET, DEVELOPER_OPTIONS,
			gettext_noop("Set this to force all raw parse trees for DML statements to be scanned by raw_expression_tree_walker(), to facilitate catching errors and omissions in that function."),
			NULL,
			GUC_NOT_IN_SAMPLE
		},
...
{
	{
		{"archive_timeout", PGC_SIGHUP, WAL_ARCHIVING,
			gettext_noop("Sets the amount of time to wait before forcing a switch to the next WAL file."),
...

	{
		{"post_auth_delay", PGC_BACKEND, DEVELOPER_OPTIONS,
			gettext_noop("Sets the amount of time to wait after authentication on connection startup."),
...

	{
		{"reserved_connections", PGC_POSTMASTER, CONN_AUTH_SETTINGS,
			gettext_noop("Sets the number of connection slots reserved for roles with privileges of pg_use_reserved_connections."),
			NULL
		},
		&ReservedConnections,
		0, 0, MAX_BACKENDS,
...

	{
		{"logical_decoding_work_mem", PGC_USERSET, RESOURCES_MEM,
			...
			gettext_noop("This much memory can be used by each internal reorder buffer before spilling to disk."),
			GUC_UNIT_KB
		},
...

	{
		{"max_locks_per_transaction", PGC_POSTMASTER, LOCK_MANAGEMENT,
			...
			gettext_noop("The shared lock table is sized on the assumption that at most \"max_locks_per_transaction\" objects per server process or prepared transaction will need to be locked at any one time.")
		},
		&max_locks_per_xact,
		64, 10, INT_MAX,
		NULL, NULL, NULL
...

	{
		{"max_pred_locks_per_transaction", PGC_POSTMASTER, LOCK_MANAGEMENT,
			...
			gettext_noop("The shared predicate lock table is sized on the assumption that at most \"max_pred_locks_per_transaction\" objects per server process or prepared transaction will need to be locked at any one time.")
		},
...

	{
		{"pre_auth_delay", PGC_SIGHUP, DEVELOPER_OPTIONS,
			gettext_noop("Sets the amount of time to wait before authentication on connection startup."),
			gettext_noop("This allows attaching a debugger to the process."),
			GUC_NOT_IN_SAMPLE | GUC_UNIT_S
		},
		&PreAuthDelay,
...

	{
		{"checkpoint_warning", PGC_SIGHUP, WAL_CHECKPOINTS,
			gettext_noop("Sets the maximum time before warning if checkpoints triggered by WAL volume happen too frequently."),
			gettext_noop("Write a message to the server log if checkpoints caused by the filling of WAL segment files happen more frequently than this amount of time. 0 disables the warning."),
			GUC_UNIT_S
		},
		&CheckPointWarning,
		30, 0, INT_MAX,
...

	{
		{"max_slot_wal_keep_size", PGC_SIGHUP, REPLICATION_SENDING,
			...
			gettext_noop("Replication slots will be marked as failed, and segments released for deletion or recycling, if this much space is occupied by WAL on disk. -1 means no maximum."),
			GUC_UNIT_MB
		},
...

	{
		{"idle_replication_slot_timeout", PGC_SIGHUP, REPLICATION_SENDING,
			gettext_noop("Sets the duration a replication slot can remain idle before it is invalidated."),
			NULL,
			GUC_UNIT_S
		},
...

	{
		{"commit_siblings", PGC_USERSET, WAL_SETTINGS,
			gettext_noop("Sets the minimum number of concurrent open transactions required before performing \"commit_delay\"."),
			NULL
		},
		&CommitSiblings,
		5, 0, 1000,
...

	{
		{"log_min_duration_sample", PGC_SUSET, LOGGING_WHEN,
			gettext_noop("Sets the minimum execution time above which a sample of statements will be logged. Sampling is determined by \"log_statement_sample_rate\"."),
...

	{
		{"log_min_duration_statement", PGC_SUSET, LOGGING_WHEN,
			gettext_noop("Sets the minimum execution time above which all statements will be logged."),
...

	{
		{"log_parameter_max_length", PGC_SUSET, LOGGING_WHAT,
			gettext_noop("Sets the maximum length in bytes of data logged for bind parameter values when logging statements."),
...

	{
		{"log_parameter_max_length_on_error", PGC_USERSET, LOGGING_WHAT,
			gettext_noop("Sets the maximum length in bytes of data logged for bind parameter values when logging statements, on error."),
			gettext_noop("-1 means log values in full."),
			GUC_UNIT_BYTE
		},
...

	{
		{"log_rotation_age", PGC_SIGHUP, LOGGING_WHERE,
			gettext_noop("Sets the amount of time to wait before forcing log file rotation."),
...

	{
		{"log_rotation_size", PGC_SIGHUP, LOGGING_WHERE,
			gettext_noop("Sets the maximum size a log file can reach before being rotated."),
...

	{
		{"tcp_keepalives_count", PGC_USERSET, CONN_AUTH_TCP,
			...
			gettext_noop("Number of consecutive keepalive retransmits that can be lost before a connection is considered dead. 0 means use the system default.")
		},
...

	{
		...
		DEFAULT_DEBUG_DISCARD_CACHES, MIN_DEBUG_DISCARD_CACHES, MAX_DEBUG_DISCARD_CACHES,
		NULL, NULL, NULL
	},
...

	{
		{"log_startup_progress_interval", PGC_SIGHUP, LOGGING_WHEN,
			gettext_noop("Time between progress updates for long-running startup operations."),
...
struct config_real ConfigureNamesReal[] =
{
...

	{
		{"parallel_tuple_cost", PGC_USERSET, QUERY_TUNING_COST,
			gettext_noop("Sets the planner's estimate of the cost of passing each tuple (row) from worker to leader backend."),
			NULL,
			GUC_EXPLAIN
		},
		&parallel_tuple_cost,
...

	{
		{"recursive_worktable_factor", PGC_USERSET, QUERY_TUNING_OTHER,
			gettext_noop("Sets the planner's estimate of the average size of a recursive query's working table."),
			NULL,
			GUC_EXPLAIN
		},
...

	{
		{"log_transaction_sample_rate", PGC_SUSET, LOGGING_WHEN,
			...
			gettext_noop("Use a value between 0.0 (never log) and 1.0 (log all statements for all transactions).")
		},
...
struct config_string ConfigureNamesString[] =
{
...

	{
		{"createrole_self_grant", PGC_USERSET, CLIENT_CONN_STATEMENT,
			gettext_noop("Sets whether a CREATEROLE user automatically grants the role to themselves, and with which options."),
...

	{
		{"extension_control_path", PGC_SUSET, CLIENT_CONN_OTHER,
			...
			gettext_noop("The remaining extension script and secondary control files are then loaded from the same directory where the primary control file was found."),
			GUC_SUPERUSER_ONLY
		},
...

	{
		{"log_destination", PGC_SIGHUP, LOGGING_WHERE,
			...
			gettext_noop("Valid values are combinations of \"stderr\", \"syslog\", \"csvlog\", \"jsonlog\", and \"eventlog\", depending on the platform."),
			GUC_LIST_INPUT
		},
...

	{
		{"unix_socket_group", PGC_POSTMASTER, CONN_AUTH_SETTINGS,
			...
			gettext_noop("The owning user of the socket is always the user that starts the server. An empty string means use the user's default group.")
		},
		&Unix_socket_group,
		"",
		NULL, NULL, NULL
...

	{
		{"synchronized_standby_slots", PGC_SIGHUP, REPLICATION_PRIMARY,
			gettext_noop("Lists streaming replication standby server replication slot names that logical WAL sender processes will wait for."),
			gettext_noop("Logical WAL sender processes will send decoded changes to output plugins only after the specified replication slots have confirmed receiving WAL."),
			GUC_LIST_INPUT
		},
...
struct config_enum ConfigureNamesEnum[] =
{
...

	{
		{"debug_parallel_query", PGC_USERSET, DEVELOPER_OPTIONS,
			...
			gettext_noop("This can be useful for testing the parallel query infrastructure by forcing the planner to generate plans that contain nodes that perform tuple communication between workers and the main process."),
			GUC_NOT_IN_SAMPLE | GUC_EXPLAIN
		},
...

	{
		{"debug_logical_replication_streaming", PGC_USERSET, DEVELOPER_OPTIONS,
			...
			gettext_noop("On the publisher, it allows streaming or serializing each change in logical decoding. On the subscriber, it allows serialization of all changes to files and notifies the parallel apply workers to read and apply them at the end of the transaction."),
			GUC_NOT_IN_SAMPLE
		},
